...
class Run(object):
    ...
    def __populate_data_from_line(self, line, trace_class_for_unique_word,
                                  window=(0, None), abs_window=(0, None)):
        """Append to trace data from a txt trace line"""

        for unique_word, cls in trace_class_for_unique_word.iteritems():
    ...
    def __parse_trace_file(self, window, abs_window, raw=False):
        ...
        for trace_name in self.class_definitions.iterkeys():
            ...
            trace_class_for_unique_word[unique_word] = trace_class

        if len(trace_class_for_unique_word) == 0:
            return

        def contain_unique_word(line, unique_words=trace_class_for_unique_word.keys()):
        ...
        with open(trace_file) as fin:
            ...
            for line in ifilter(contain_unique_word, fin):
                try:
                    self.__populate_data_from_line(line,
                                                   trace_class_for_unique_word,
                                                   window, abs_window)
